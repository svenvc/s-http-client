...
(defvar *http-client-accept-encoding*
  nil ;; disabled for now, set to "gzip" to enable
  "The value to use for the 'Accept-Encoding' request header")

;; special variable to pass endpoint info down the call chain
...
(define-condition http-client-error (error)
  ((endpoint :accessor get-endpoint :initarg :endpoint :initform nil)
   (timeout :accessor get-timeout :initarg :timeout :initform nil)))

(define-condition http-client-connection-error (http-client-error)
  ()
  (:report (lambda (condition stream)
             (with-slots (endpoint timeout)
                 condition
               (format stream "Cannot establish http client endpoint ~s~@[ within ~d second~:p~]"
                       endpoint timeout)))))

(define-condition http-client-read-error (http-client-error)
  ()
  (:report (lambda (condition stream)
             (with-slots (endpoint timeout)
                 condition
               (format stream "Cannot read from http client endpoint ~s~@[ within ~d second~:p~]"
                       endpoint timeout)))))

(define-condition http-client-write-error (http-client-error)
  ()
  (:report (lambda (condition stream)
             (with-slots (endpoint timeout)
                 condition
               (format stream "Cannot write to http client endpoint ~s~@[ within ~d second~:p~]"
                       endpoint timeout)))))
...
(defun get-open-connection (scheme host port state &key force-new connect-timeout read-timeout write-timeout)
  ...
  (let ((scheme-host-port (format nil "~a://~a:~d" (string-downcase scheme) host port)))
    (flet ((make-new-connection () (or (open-socket-stream scheme host port
                                                           :connect-timeout connect-timeout 
                                                           :read-timeout read-timeout 
                                                           :write-timeout write-timeout)
                                       (error (make-condition 'http-client-connection-error 
                                                              :endpoint *endpoint* :timeout connect-timeout))))
           (make-new-buffer () (make-string 4096)))
      (if state
          (s-sysdeps:with-process-lock ((get-lock state))
            (let* ((server-state (get-http-server-state state scheme-host-port))
              ...
              (cond ((get-in-use-p server-state)
                     ...
                     (values (make-new-connection) :new (make-new-buffer)))
                    ((and connection (open-stream-p connection) (not force-new))
                     ...
                     ;; so reuse it after marking it in-use and setting the read & write timeouts
                     (setf (get-in-use-p server-state) t)
                     #+lispworks (setf (stream:stream-read-timeout connection) read-timeout
                                       (stream:stream-write-timeout connection) write-timeout) 
                     (values connection :keep-alive (get-buffer server-state)))
                    (t
                     ;; either we didn't find a good connection or were forced to use a new one
                     ;; so we have to open a new one and store it for later
                     (close-connection connection)
                     (setf (get-in-use-p server-state) t)
                     (values (setf (get-socket server-state) (make-new-connection)) :new (get-buffer server-state))))))
...
(defun write-http-request-line (string stream)
  (handler-case (progn
                  (write-string string stream)
                  (write-char #\return stream)
                  (write-char #\linefeed stream))
    (error (condition)
      (error (make-condition 'http-client-write-error 
                             :endpoint *endpoint* :timeout #+lispworks(stream:stream-write-timeout stream))))))
  ...
  (let ((offset 0)
    ...
    (handler-case 
      ...
      (end-of-file (condition) 
        (error (make-condition 'http-client-read-error
                               :endpoint *endpoint* :timeout #+lispworks(stream:stream-read-timeout stream)))))))
  ...
  (let (response-code response-headers response-body)
    ...
    (when (and body-expected (not (= response-code 204)))
      (let* ((content-length-header (cdr (find :content-length response-headers :key #'car)))
             ...
             (content-encoding-header (cdr (find :content-encoding response-headers :key #'car)))
             (gzipped-p (when content-encoding-header
                          (search "gzip" content-encoding-header :test #'char-equal))))
        ...
        (when gzipped-p
          (setf response-body (gunzip response-body)))))
...
(defun write-request (stream buffer
  ...
  (when *http-client-accept-encoding*
    (format-http-request-line stream "Accept-Encoding: ~a" *http-client-accept-encoding*))
...
(defun do-http-request-internal (scheme host port state 
  ...
  (let ((*endpoint* (princ-to-string uri)))
    (multiple-value-bind (connection keep-alive buffer)
      ...
      (labels ((execute-request-response ()
                 (values-list `(,@(multiple-value-list 
                                   (do-one-request-response connection uri method buffer 
                                                            ...
                                                            :headers headers)) 
                                ,keep-alive)))
               (execute-request-response-and-close-on-error ()
                 (unwind-protect 
                     (execute-request-response)
                   (close-connection connection))))
        (unwind-protect
            (if (eql keep-alive :new)              
                (execute-request-response-and-close-on-error)
              (handler-case (execute-request-response)
                ;; the first stream/socket error on the re-used (kept-alive) connection is interpreted 
                ;; as a failure to re-use the kept-alive connection, so we close the connection and retry once again
                ((or http-client-error stream-error #+lispworks comm:socket-error) ()
